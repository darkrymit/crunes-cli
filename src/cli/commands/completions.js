...
export function resolveCompletions(tokens, program, { cwd } = {}) {
  ...
  if (!activeOpt && prevWord === '--format' && ['run', 'check', 'bench'].includes(currentCmd.name())) {
  ...
  if (argDef) {
    ...
    else if (argName === 'args' && positionalArgs.length === 0 && ['run', 'check', 'bench'].includes(currentCmd.name())) {
