...
export async function compileIntro({ config, format, projectRoot, configRoot, hasProjectError }) {
  ...
  lines.push('```')
  lines.push('')
  lines.push('For runes that use both `run` and `runRepl`, declare both permission namespaces separately — `runRepl` does not inherit from `run`:')
  lines.push('```json')
  lines.push('{')
  lines.push('  "runes": {')
  lines.push('    "my-shell": {')
  ...
  lines.push('        "runRepl": { "allow": ["sqlite.read:./state::db", "sqlite.write:./state::db"] }')
  lines.push('      }')
  lines.push('    }')
  lines.push('  }')
  lines.push('}')
