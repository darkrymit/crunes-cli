...
export async function compileIntro({ config, format, projectRoot, configRoot, hasProjectError }) {
  ...
  lines.push('- **Named Positionals**: Parameters like \`<name>\` or \`[scope]\` are automatically mapped to \`args.name\` or \`args.scope\`, with offsets automatically calculated based on matched command depth.')
  lines.push('')
