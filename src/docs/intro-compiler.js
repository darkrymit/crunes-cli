...
export async function compileIntro() {
  ...
  lines.push('')
  lines.push('Scaffold a new rune with `crunes create`. In non-interactive/agent sessions `--format` is required (no default); `--path` defaults to `.crunes/runes/<key>.js` if omitted:')
  lines.push('```bash')
  lines.push('crunes create my-rune --format markdown --yes')
  lines.push('```')
  lines.push('')
