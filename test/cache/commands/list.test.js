...
describe('cache list handler', () => {
  ...
  it('project-scoped: shows own bucket, hides other-project bucket', async () => {
    const { id: myKey } = await ensureProjectIdentity(tmp)
    const myPath = join(tmp, 'caches', 'projects', myKey, 'default')
    await upsertCacheBucket(myPath, { scope: 'global-project', projectId: myKey, pluginId: null, location: '@global-project-cache', name: 'default' })
    const otherPath = join(tmp, 'caches', 'projects', 'other-key', 'other')
    await upsertCacheBucket(otherPath, { scope: 'global-project', projectId: 'other-key', pluginId: null, location: '@global-project-cache', name: 'other' })
    await handler({ projectDir: tmp, global: false })
  ...
  it('-g: shows all buckets including other projects', async () => {
    ...
    await upsertCacheBucket(p1, { scope: 'global-project', projectId: 'proj-a', pluginId: null, location: '@global-project-cache', name: 'a' })
    await upsertCacheBucket(p2, { scope: 'global-project', projectId: 'proj-b', pluginId: null, location: '@global-project-cache', name: 'b' })
  ...
  it('prints a table with header columns when buckets exist', async () => {
    ...
    await upsertCacheBucket(bucketPath, { scope: 'global-project', projectId: myKey, pluginId: null, location: '@global-project-cache', name: 'default' })
    await handler({ projectDir: tmp, global: false })
    const lines = console.log.mock.calls.map(c => c[0])
