...
- **A permission denial named the grant but not where to write it** — `run` and `repl` now append the exact config edit that resolves it, scoped to the lifecycle actually invoked. `shell.run` denials point at `crunes shell explain`, `fs.glob` denials state that the granted pattern is matched exactly, and an unscannable command is told no grant overrides it
- **`crunes <unknown> --help` printed the root help and exited 0**, which read as confirmation that the unknown command existed. The name is now rejected first, whatever follows it
- **`--plain` still emitted box-drawing** from the clack-rendered `plugin install`, `update`, `uninstall` and consent paths, so piped and agent-captured output carried frame characters
- **The batch denial pointed at the wrong config key** — it said "add a batch.allow pattern in config.json", which reads as `permissions.batch`. `batch` is a sibling of `permissions` on the rune entry, and its patterns match the rune's arguments (or its bare key when invoked with none); the message now says both and prints the entry to write
- **`crunes docs rune` called an absent batch block "not permitted"**, which read as a permission failure rather than a missing declaration
- **A rune-level `.example()` rendered nowhere** when the rune had no subcommands — the root is always an index, and the index withholds examples to stay bounded. With no command page to drill to, the index is the rune's whole surface, so its examples now render there
...
### Fixed
- **comment-json runtime dep**: mark comment-json as external in esbuild and move to dependencies — it uses internal CJS require() paths that cannot be statically bundled
...
### Fixed
- **esbuild runtime dep**: move esbuild back to dependencies — it is dynamically imported at runtime by the npm package resolver and must be available in global installs
...
### Changed
- **Dependencies**: move chokidar, comment-json, csv-parse, csv-stringify, json5 from dependencies to devDependencies — all are bundled by esbuild and were incorrectly listed as runtime deps
