...
* [An fs.glob grant matches the pattern string, not the paths](/gotchas/glob-grants-match-exactly.md) - The granted pattern must equal the pattern the call passes, with only fs.glob:* acting as a wildcard, so the broadest-looking grant fails while a narrower one works.
