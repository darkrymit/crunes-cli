...
**The store root is also the global config directory:** `<store>/config.json` is the lowest config layer, beneath the project config. Global runes live at `<store>/runes/<key>.js` and templates at `<store>/templates/<name>.js` — there is no `.crunes/` segment, because `~/.crunes` already *is* the config directory. This is the single most common source of confusion when writing global paths.

**Rootless redirection:** A directory with no `.crunes/config.json` is not a project. Rather than refusing to run there — or worse, scattering `.crunes/` directories across arbitrary directories — crunes redirects all local state into `<store>/rootless/<hash-of-cwd>/`. `getLocalBase(projectDir)` is the one function that decides this, returning either `<projectDir>/.crunes` or the rootless base. Every consumer of local state routes through it, so no call chain needed a new argument. The invariant it buys: **a rootless run writes nothing outside the store.** Permission paths and `@project/` still resolve against the working directory, which is what lets a personal rune act on whatever directory you invoke it from.
...
**A bare temp directory is rootless:** Any test that creates a temp directory and expects local state under `<dir>/.crunes/` must first write a `<dir>/.crunes/config.json` — without one the directory is not a project, and paths redirect into the store. This catches out tests for caches, sqlite, jobs, schemas, and `@local-*` permission patterns.
