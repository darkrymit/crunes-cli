...
`crunes template use <template>` resolves the template, copies the file to the project, and writes the rune entry into `.crunes/config.json`. `crunes template create` creates a new blank template file and registers it in the local project config.
...
**Shortcut entries:** A config entry `{ plugin: "pluginBareName:templateKey" }` is a shortcut — it points to a plugin template. The shortcut is resolved at `template use` time: `resolvePluginKey` looks up the plugin in the global registry, then `loadPluginJson` finds the template metadata. Shortcuts are not expanded at `template list` time.

**Config write in `template use`:** After copying the template file, the handler reads `.crunes/config.json`, merges `{ path, name?, description?, permissions? }` under `config.runes[outputKey]`, and writes back via atomic temp-file rename (`configPath + '.tmp'` → `configPath`). If the key already exists, it is overwritten without backup.
