...
- **LOCAL RUNES RUN INSIDE `isolated-vm`** — Local runes run in a sandboxed V8 isolate and cannot access standard Node.js built-ins directly. All I/O operations must go through `import { fs, shell, section, ... } from '@utils'`. Run `crunes -p docs utils <ns>` for the full namespace reference.
